"""3-slice variant of test_gd_msd_final_recall.py.

Reads ``optimal_threshold_3slice.{txt,json}``, loads the
``resnet_3slice_fold_*.pth`` ensemble, scores test candidates with 3-slice
context, runs MedSAM on the survivors, and writes
``data/results/dice_final_report_msd_recall_3slice.csv``.

Usage (from repo root):
    python -m experiments.three_slice.test_recall_3slice
"""
...
CHECKPOINT_PREFIX = "resnet_3slice_fold"
THRESH_TXT = "optimal_threshold_3slice.txt"
THRESH_JSON = "optimal_threshold_3slice.json"
OUT_CSV = "data/results/dice_final_report_msd_recall_3slice.csv"
...
def load_threshold_and_config():
    ...
    if os.path.exists(THRESH_JSON):
        ...
        print(f"Seuil 3-slice charge depuis {THRESH_JSON} : {threshold:.2f}")
    ...
    print(
        f"Aucun seuil 3-slice calibre trouve ({THRESH_TXT}/{THRESH_JSON}). "
        f"Seuil force a {threshold:.2f}."
    )
    return threshold, ProposalConfig()
...
for img_info in tqdm(test_images_list, desc="Inference Test 3-slice"):
...
print("\n" + "=" * 50)
print(f"RESULTATS FINAUX 3-SLICE (Seuil = {OPTIMAL_THRESH:.2f})")
print("=" * 50)
