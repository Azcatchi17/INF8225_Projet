"""3-slice variant of extract_hard_negatives.py.
...
folder (`data/classifier_dataset_3slice/`) so the original 2D dataset is
preserved.

Usage (from repo root):
    python -m experiments.three_slice.extract_hard_negatives_3slice
"""
...
# Make repo root importable when launched from anywhere
...
# ==========================================
# CONFIG (mirrors extract_hard_negatives.py)
# ==========================================
device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"Extraction 3-slice sur : {device}")
...
output_base_dir = "data/classifier_dataset_3slice"
...
def crop_3slice(stacked_img: np.ndarray, box, margin: int) -> np.ndarray | None:
    """Crop the same box from a pre-stacked 3-channel image."""
...
def extract_hard_patches(split_name):
    print(f"\n--- Traitement du set : {split_name} (3-slice) ---")
    ...
    for img_info in tqdm(split_data["images"], desc=f"Extraction {split_name}"):
        ...
        # Build the 3-channel stack ONCE per image (heaviest IO step)
