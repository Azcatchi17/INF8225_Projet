"""3-slice v2 training: ResNet-50 ImageNet pretrained on the wider 3-slice dataset.

ResNet-50 (~25M params) replaces ResNet-18 (~11M) so the classifier has
enough capacity to exploit the richer features that ImageNet pretraining
provides. Training stays at 20 epochs with the same optimizer schedule,
but we drop ``BATCH_SIZE`` from 32 to 16 because the heavier backbone
doubles the activation memory.

Checkpoints are saved with the prefix ``resnet50_3slice_fold_*.pth``.

Usage (from repo root):
    python -m experiments.three_slice_v2.train_v2
"""
...
# Smaller batch because ResNet-50 + 3-channel inputs use more memory than ResNet-18
BATCH_SIZE = 16
NUM_RUNS = 5
EPOCHS = 20
MIN_PRECISION = 0.80
...
CHECKPOINT_PREFIX = "resnet50_3slice_fold"
THRESHOLD_PREFIX = "threshold_3slice_v2_run"

TRAIN_DIR = "data/classifier_dataset_3slice_v2/train"
VAL_DIR = "data/classifier_dataset_3slice_v2/val"
...
# Same Normalize stats as v1: each channel is a CT-window grayscale slice
# with similar intensity statistics, so [0.5,0.5,0.5] is fine.
...
for run in range(1, NUM_RUNS + 1):
    set_seed(1000 + run)
    print("\n" + "=" * 40)
    print(f"LANCEMENT DU MODELE 3-SLICE v2 (RN-50) {run}/{NUM_RUNS}")
    print("=" * 40)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch >= WARMUP_EPOCHS:
            ...
            if is_better:
                ...
                print("  -> Nouveau meilleur modele 3-slice v2 sauvegarde !")
...
print("\n" + "=" * 40)
print("BILAN DE L'ENSEMBLE 3-SLICE v2 (5 ResNet-50)")
print("=" * 40)
