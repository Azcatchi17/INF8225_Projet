"""3-slice v2 calibration: ResNet-50 ensemble + crop_margin=30.

Loads ``resnet50_3slice_fold_*.pth``, scores validation candidates with
3-slice context and the wider crop, and writes
``optimal_threshold_3slice_v2.{txt,json}``.

Usage (from repo root):
    python -m experiments.three_slice_v2.calibrate_v2
"""
...
from experiments.three_slice.score_3slice import score_candidates_3slice
from experiments.three_slice.slice_stack import stack_3slice_image


CHECKPOINT_PREFIX = "resnet50_3slice_fold"
OUT_TXT = "optimal_threshold_3slice_v2.txt"
OUT_JSON = "optimal_threshold_3slice_v2.json"
OUT_CSV = "data/results/calibration_threshold_3slice_v2.csv"
...
# Same proposal logic as v1, but bump crop_margin to 30 to match the
# patches the ResNet-50 was trained on.
...
print(
    f"\nDebut de la calibration 3-slice v2 sur {len(val_data['images'])} images de validation..."
)
rows = []

for img_info in tqdm(val_data["images"], desc="Calibration Val Set v2"):
...
print("\n" + "=" * 58)
print("RECHERCHE DU SEUIL OPTIMAL 3-SLICE v2 (ResNet-50)")
print("=" * 58)
