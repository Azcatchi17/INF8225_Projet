...
```bash
python -m experiments.msd.resnet18_recall.calibrate_detector
python -m experiments.msd.resnet18_recall.extract_hard_negatives
python -m experiments.msd.resnet18_recall.train_classifier
python -m experiments.msd.resnet18_recall.calibrate_threshold
python -m experiments.msd.resnet18_recall.evaluate
```

The same steps in Colab format are under
`notebooks/msd/resnet18_recall/`.
...
key analysis is in `experiments/msd/_shared/RECALL_STRATEGY.md` and
