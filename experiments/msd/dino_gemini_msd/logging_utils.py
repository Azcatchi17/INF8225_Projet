"""Per-iteration JSON writer + pickled AgentState for resumability."""
...
def write_iteration(state: AgentState, iteration_idx: int) -> Path:
    """Dump a single iteration result as JSON next to the run."""
...
def write_run_summary(state: AgentState) -> Path:
    """Full state (no numpy fields) serialised as JSON — overwritten each step."""
    d = _run_dir(state.run_id)
    path = d / "run.json"
    state.to_json(path)
    return path
...
def pickle_state(state: AgentState) -> Path:
    """Pickle the AgentState (minus tensors) so a crashed run can be resumed."""
...
def log_run(state: AgentState) -> dict:
    """Run full logging at the end of an agent loop. Returns the paths."""
