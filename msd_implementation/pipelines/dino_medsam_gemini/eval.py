"""Batch evaluation + one-shot vs agentic comparison (MSD Pancreas version)."""
...
def _load_gt(file_name: str) -> np.ndarray:
    """Charge le masque GT et isole la tumeur (Label 2 pour MSD)"""
    # Remplacement dynamique du dossier images par masks
    mask_rel_path = file_name.replace("/images/", "/masks/")
    mask_path = config.MSD_MASKS / mask_rel_path
    
    mask = io.imread(mask_path)
    if mask.ndim == 3:
        mask = mask[..., 0]
    
    # CRITIQUE : on isole uniquement la valeur 2 (tumeur)
...
def run_batch(
    test_json: str | Path = config.MSD_TEST_JSON,
    user_text: str = "find the pancreatic tumor", # Nouveau prompt par défaut
    n: Optional[int] = None,
    ...
    for img_info in tqdm(images, desc="agentic"):
        ...
        img_path = config.MSD_IMAGES / file_name
